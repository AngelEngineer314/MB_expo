...
class WastageScreen extends React.Component {
  ...
  getData(){
    ...
    AsyncStorage.getItem('@app:token').then(authToken => {
      ...
      AsyncStorage.getItem('@app:user_id').then(userId => {
        ...
        fetch(Config.apiUrl + '/wastage/check',{
          method: 'POST',
          headers: {
  ...
  extractEntries = (data) => {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (index>=0){
        entries[index] = {'initial': data[i].initial,'wastage': parseInt(entries[index].wastage + parseInt(data[i].wastage))};
      }
      else{
        const obj = {'initial':data[i].initial, 'wastage':parseInt(data[i].wastage)};
        entries.push(obj);
      }
    }
    entries.map(function(item) {
      strEntries.push(item['initial']+'/'+item['wastage']);
    });
  ...
  saveData(){
    ...
    AsyncStorage.getItem('@app:user_id').then(userId => {
      ...
      return fetch(Config.apiUrl + '/wastages/store',{
          method: 'POST',
            headers: {
              'Accept': 'application/json',
              'content-type': 'application/json',
  ...
  _renderItem = ({ item }) => (
    <WastageView
      ...
      hasComment={item[1].projections.length>0 ? true : false}
      entries={this.extractEntries(item[7].wastages)}
  ...
  render() {
    ...
    color: "#230A59", textAlign: 'center', fontFamily:AppStyles.fontName.bold}}>WASTAGE</Text>
