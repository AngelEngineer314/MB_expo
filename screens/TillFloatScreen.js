...
class TillFloatScreen extends React.Component {

  static navigationOptions = {  
    headerTitle: 'Till Float',
  };  
  ...
  getData(){
    ...
    AsyncStorage.getItem('@app:business').then((response) => {
      ...
      CounterScreen.fetchBusiness(responseJson.data.counter.id).then(businessResponse => {
        const tillFloat = businessResponse.data.tillFloat;
        const counter = businessResponse.data.counter;

        const minTotal = parseFloat(counter.till_float) - parseFloat(counter.fluctuation);
        const maxTotal = parseFloat(counter.till_float) + parseFloat(counter.fluctuation);

        if (tillFloat) {
          this.setState({
            N100:tillFloat.notes_100,
            N50:tillFloat.notes_50,
            N20:tillFloat.notes_20,
            N10:tillFloat.notes_10,
            N5:tillFloat.notes_5,
            C2:tillFloat.coins_2,
            C1:tillFloat.coins_1,
            C05:tillFloat.coins_05,
            C02:tillFloat.coins_02,
            C01:tillFloat.coins_01,
            C005:tillFloat.coins_005,
            NTotal:parseInt(tillFloat.notes_total),
            CTotal:parseFloat(tillFloat.coins_total),
            T100: parseInt(100*tillFloat.notes_100),
            T50: parseInt(50*tillFloat.notes_50),
            T20: parseInt(20*tillFloat.notes_20),
            T10: parseInt(10*tillFloat.notes_10),
            T5: parseInt(5*tillFloat.notes_5),
            T2: parseInt(2*tillFloat.coins_2),
            T1: parseInt(1*tillFloat.coins_1),
            T05: this.parseFloat2Decimals(0.5*tillFloat.coins_05),
            T02: this.parseFloat2Decimals(0.2*tillFloat.coins_02),
            T01: this.parseFloat2Decimals(0.1*tillFloat.coins_01),
            T005: this.parseFloat2Decimals(0.05*tillFloat.coins_005),
          }); 
        }
        this.setState({
          isLoading: false,
  ...
  onPressSave = () => {
    ...
    if ((this.state.NTotal+this.state.CTotal)==0) {
      message = 'Please enter some values';
      buttons = [{text: 'Ok', style: 'cancel'}];
    }else if((parseInt(this.state.NTotal)+this.parseFloat2Decimals(this.state.CTotal))>this.state.maxTotal){
      message = "Total cannot be more than "+this.state.maxTotal;
      buttons = [{text: 'Ok', style: 'cancel'}];
    }else if((parseInt(this.state.NTotal)+this.parseFloat2Decimals(this.state.CTotal))<this.state.minTotal){
      message = "Total cannot be less than "+this.state.minTotal;
      buttons = [{text: 'Ok', style: 'cancel'}];
    }
...
export default TillFloatScreen;
